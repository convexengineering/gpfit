...
def LM(residfun,initparams):
	...
	while True:
		...
		if trialrms < rms:
			...
			#dsp here so that all grad info is for updated point, but lambda not yet updated
			if options['bverbose']:
				print formatstr % (itr, trialrms, maxgrad, lamb, norm(step), max(diagJJ)/min(diagJJ))
